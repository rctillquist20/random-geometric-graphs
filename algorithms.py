...
def randomSet(G):
    ...
    G = [[int(G[u][v]) if (v in G and G[u][v] != np.inf)
...
#  components = sorted([c for c in nx.connected_components(G)], key=len)
    ...
    for i, H in enumerate(components):
        if len(H) == 1 and not iso:
            iso = True
        elif len(H) == 1 and iso:
            R.append(next(iter(H)))
#    if len(H) == 1 and i > 0: R.append(next(iter(H)))
        elif len(H) == 2:
            R.append(next(iter(H)))  # list(H)[0]
        else:
            R.extend([sorted(list(H))[r] for r in ich(G.subgraph(H))])  # i > 0
    return R
...
def plotDiffs(ichInfo, algoInfo):
    ...
    for (n, r, seed, resSet, t) in ichInfo:
        if (n, r, seed) not in matched:
            matched[(n, r, seed)] = {'ich': [], 'algo': []}
        matched[(n, r, seed)]['ich'] += [len(resSet), t]
    for (n, r, seed, resSet, t) in algoInfo:
        if (n, r, seed) not in matched:
            matched[(n, r, seed)] = {'ich': [], 'algo': []}
        matched[(n, r, seed)]['algo'] += [len(resSet), t]
    resSetDiffs = [matched[combo]['ich'][0] -
                   matched[combo]['algo'][0] for combo in matched]
    timeDiffs = [matched[combo]['ich'][1] - matched[combo]['algo'][1]
...
#  print('time diff counts', Counter(timeDiffs))
    ...
    plt.xlabel('Difference (ICH - Random)')
...
def saveResults(data, algo=ichComponents, saveFile=''):
    if not saveFile:
        saveFile = 'rgg_ichComponents_2.list'
    results = readList(saveFile) if glob.glob(saveFile) else []

    complete = set([(n, r, seed) for (n, r, seed, _, _) in results])
    jobs = [(n, r, seed, resSet, t)
            for (n, r, seed, resSet, t) in data if (n, r, seed) not in complete]
...
if __name__ == '__main__':
    dataFile = 'rgg_below_thresh.list'
    # print(decode.get_items_list(dataFile, nodes=True, radius=True))
    results = decode.get_items_list(dataFile, nodes=True)
    dog = 0
    for n in results:
      print(n)
      if n == 1000:
    ...
    # data = [values for values in data if values[1] <= np.sqrt(
    #     np.log(values[0])/(np.pi*values[0]))]  # and values[0] <= 100]
    ...
    # cool = 0
    # for (n, r, seed, R, t) in sorted(results):
    #     # print(n, r, len(R), t)
    #     if n == 100:
    #       cool += 1
    
    # print('TOTAL SCORE 100:', cool)
    # rad = np.random.choice([v for (_, v, _, _, _) in data])
    # print('radius', rad)
    # print('ICH', np.mean([len(R) for (_, r, _, R, _) in data if r == rad]), 'Algo', np.mean(
    #     [len(R) for (_, r, _, R, _) in results if r == rad]))
