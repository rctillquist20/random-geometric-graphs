...
class ZoomedSquareCollisionMethodTests(unittest.TestCase):
    ...
    def test_entropy_control_matches_existing_zoomed_square(self):
        graph = nx.random_geometric_graph(35, 0.4, seed=991)

        random.seed(73)
        original = get_metric_dimension_of_graph_with_pruning_igraph_zoomed_square(
            graph,
            0.4,
            k_nearest=5,
        )
        random.seed(73)
        control = get_metric_dimension_zoomed_square_entropy_experimental(
            graph,
            0.4,
            k_nearest=5,
        )
