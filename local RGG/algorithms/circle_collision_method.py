"""Isolated Circle experiment for entropy versus collision scoring.

The existing :mod:`circle_method` is intentionally left unchanged. Both
selectors in this module share its center selection, one-hop neighborhoods,
circle probe generation, candidate pools, stopping rule, and final pruning so
the local candidate score is the controlled difference.
"""
...
from algorithms.circle_method import UnitCircle
...
def _resolve_circle_neighborhood(
    g,
    radius,
    center,
    dist_matrix,
...
def _run_circle_selector(
    ...
    while iter_count < max_iters:
        ...
        local_set, nodes_set = _resolve_circle_neighborhood(
            g,
            radius,
            center,
            dist_matrix,
    ...
    return CircleSelectionRun(resolving_set, pre_prune_size)


def get_metric_dimension_circle_entropy_experimental(
    graph,
    radius,
    k_nearest=5,
    max_iters=1000,
):
    """Run the isolated Circle control with entropy scoring."""

    return _run_circle_selector(
        graph,
        radius,
        k_nearest,
        "entropy",
...
def get_metric_dimension_circle_collision(
    graph,
    radius,
    k_nearest=5,
    max_iters=1000,
):
    """Run Circle with collision-minimizing candidate selection."""
